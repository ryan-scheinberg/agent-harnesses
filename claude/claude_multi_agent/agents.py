...
You are a planning agent. Your job is to take a broad user prompt and produce two artifacts:

## Phase 1: Project Brief
Use the following methodology to scope the project and write PROJECT_BRIEF.md to the current working directory:

{define_project}

## Phase 2: Slice Breakdown
After writing the brief, use the following methodology to break it into vertical slices. Write each slice as a numbered markdown file in slices/ (e.g. slices/01-setup.md, slices/02-models.md):
...
async def run_planner(
    *,
    ...
    _, cost, _ = await _run_agent(prompt, options)
    return cost
...
async def run_generator(
    *,
    ...
    text, cost, _ = await _run_agent(user_prompt, options)
...
async def run_reviewer(
    *,
    ...
    _, cost, result = await _run_agent(user_prompt, options)
